...
def parse_arguments():
    ...
    parser.add_argument('-m', '--map-name', action='append', required=True)
    parser.add_argument('-r', '--region', action='store', )                 # make default if it is in ~/.aws/config

    args = parser.parse_args()

    if args.image_id:
        if len(args.image_id) != len(args.map_name):
            parser.error("Number of -i/--image-id should be equal to number of -m/--map-name")
    elif args.image_name:
        if len(args.image_name) != len(args.map_name):
            parser.error("Number of -n/--image-name should be equal to number of -m/--map-name")
